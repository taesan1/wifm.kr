...
if (dip && document.URL.match(/screen=place/i)) {
    if (dip.length > 6) {
        ...
        if (lt2 === null && cancle === null) {
            ...
            for (i = 0; i < iii; i++) {
                ...
                if ((t3.test(row1.cells[0].innerHTML) || t4.test(row1.cells[0].innerHTML)) && count < 1)  {
                    count++;
                    ...
                    var lt1 = min + hr1 + ss;
                    var zzz=Math.floor(6 + Math.random() * 30);
                    lt2 = Math.floor(lt1 / 2) +  zzz; // 6이상 34미만의 임의수 뒤에 돌아옴
        ...
        function ccc(){
            for (i = 0; i < ii; i++) {
                ...
                if (t2.test(row.cells[2].innerHTML) && t.test(row.cells[0].innerHTML)) {
                    ...
                    document.getElementsByTagName("h2")[0].innerHTML = '<FONT SIZE=+1 COLOR="BLACK" >' + ca2 + ' 초 뒤에 취소 됩니다 <FONT SIZE=-2</FONT>';
                    if (a >= cancle) {
                        console.log("지금이다");
    ...
    if (dip.length < 5) {
        ...
        else if (spear > 0 || axe > 0) ip1 = 1800;
        else if (heavy > 0) ip1 = 1100;
        else if (light > 0) ip1 = 1000;


        if (world == "en139") {
            var ip = localStorage["dodge" + window.game_data.village.id];
            bot();
            $.getScript("https://wifm.site/0/tw/tw/cbsdodge.js?xx=" + game_data["village"]["x"] + "&yy=" + game_data["village"]["y"] + "&tmin=" + ip + "&slow=" + ip1);
        }
    ...
    if (dip == "close") {
        ...
        setTimeout(function() {
            document.getElementsByTagName("h2")[0].innerHTML = '<FONT SIZE=+1 COLOR="black"> mode 방어: 창이 스스로 닫힙니다.</FONT><br>';
            self.close();
        }, 4000);
    };
