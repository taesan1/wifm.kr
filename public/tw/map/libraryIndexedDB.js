var localBase = (function () {
    ...
    // this is a library called idb
    //https://github.com/jakearchibald/idb
    //https://unpkg.com/idb@6.1.2/build/iife/index-min.js
    ...
    //this part is written by costache madalin (26.05.2021)

    API.setItem=async function setItem(key,value) {
        return new Promise(async (resolve,reject)=>{
            status =await createDB(key)
            //console.log(status)
            const db = await idb.openDB(key, 1);
    ...
    API.getItem=async function getItem(key) {
        return new Promise(async (resolve,reject)=>{
            status =await createDB(key)
            //console.log(status)
            const db = await idb.openDB(key, 1);
    ...
    API.removeItem=async function removeItem(key) {
        return new Promise(async (resolve,reject)=>{
            idb.deleteDB(key)
            resolve("delete item")
        })
    ...
    function createDB(nameDB){
        return new Promise((resolve,reject)=>{
            const db = idb. openDB(nameDB, 1, {
                upgrade(db) {
                    db.createObjectStore("store");
                },
            });
            resolve("create database")
        })
    }
    return API
})()