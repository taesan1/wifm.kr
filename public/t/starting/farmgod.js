...
window.FarmGod.Main = (function (Library, Translation) {
        ...
        if (game_data.features.Premium.active && game_data.features.FarmAssistent.active) {
            if (game_data.screen == 'am_farm') {
                $.when(buildOptions()).then((html) => {
                    ...
                    bot();
                    localStorage.getItem('farmrest')||0;
                    const storedOptions = localStorage.getItem('farmGod_options');
                    if (storedOptions) {
                        a();
                        UI.InfoMessage("버튼이 클릭 된 후 25초간 대기합니다.. ",2000);
                    }
    ...
    function a(){
        UI.InfoMessage("버튼 클릭", 4000);
        localStorage.setItem('farmrest', 'wait');
        setTimeout(() => {
            $('.optionButton').click();
            console.log("버튼 클릭 ");
        }, 5000); // 2000ms (2초) 후에 실행합니다.
    }
    ...
    const buildOptions = function () {
        let options = JSON.parse(localStorage.getItem('farmGod_options')) || { optionGroup: 0, optionDistance: 30, optionTime: 10, optionLosses: true, optionMaxloot: true, optionNewbarbs: true };
    ...
    let currentIndex = 0; // 현재 처리 중인 farmGod_icon의 인덱스

    const checkAndContinueFarming = function() {
        let $nextFarmButton = $('.farmGod_icon').first(); // 다음으로 클릭할 버튼을 찾습니다.
        if ($nextFarmButton.length > 0&& $('.farmRow').length > 0) {
            $nextFarmButton.click();
        } else {
            setTimeout(function() {
                localStorage.setItem('farmrest', 'done');
                var world1 = game_data.world;
                var vid1 = game_data.village.id;
                window.location.href = "https://" + world1 + ".tribalwars.com.br/game.php?village=" + vid1 + "&screen=am_farm";
            }, 3000); // 3초 후에 실행
        }
    }
    ...
    const sendFarm = function ($this) {
        ...
        if (!farmBusy && !(Accountmanager.farm.last_click && n - Accountmanager.farm.last_click < 200)) {
            ...
            TribalWars.post(Accountmanager.send_units_link.replace(/village=(\d+)/, 'village=' + $this.data('origin')), null, {
                ...
                farmBusy = false;
                setTimeout(function() {
                    checkAndContinueFarming(); // 다음 작업 진행
                }, 350); // 3초 후에 실행
            }, function (r) {
